...
def find_flex_course_duration(table_rows: list) -> str:
    for row in rows:
        ...
        if "Тривалість" in row.text:
            ...
            duration_cells = row.find_elements(By.CLASS_NAME, "TableColumnsView_tableCellGray__4hadg")
...
def extract_card_info(driver:WebDriver, course_card: WebElement) -> dict:
    ...
    course_name = course_card.find_element(By.CLASS_NAME, "ProfessionCard_title__m7uno").text
    full_time_course_duration = course_card.find_element(By.CLASS_NAME, "ProfessionCard_duration__13PwX").text
    course_description = course_card.find_elements(By.CLASS_NAME, "ProfessionCard_description__K8weo")[0].get_attribute("innerText").strip()
    course_link = course_card.get_attribute("href").strip()
    logging.info(f"Extracted card info for {course_name}: Full-time duration: {full_time_course_duration}, Description: {course_description}")
    return {
        "name": course_name,
...
with webdriver.Chrome() as driver:
    ...
    course_cards = driver.find_elements(By.XPATH, "//a[contains(@href, 'profession_card')]")
    logging.info(f"Found {len(course_cards)} courses.")
    for num in range(len(course_cards)):
        if num != 0:
            driver.get(BASE_URL)
            course_cards = driver.find_elements(By.CLASS_NAME, "ProfessionCard_cardWrapper__BCg0O")  # Re-find elements
        ...
        full_time_format = 1 if driver.find_elements(By.PARTIAL_LINK_TEXT, "повний день") else 0
        flex_format = 1 if driver.find_elements(By.PARTIAL_LINK_TEXT, "вільний час") else 0

        if flex_format:
            rows = driver.find_elements(By.CLASS_NAME, "TableColumnsView_contentRow__QYVPu")
            flex_course_duration = find_flex_course_duration(rows)

            logging.info(f"Flex course duration for {card_info['name']}: {flex_course_duration}")

        # Extract topic number and module count
        topic_number = driver.find_elements(By.CLASS_NAME, "CourseProgram_cards__CD13X")[0].text.split(sep='\n')[0]
        modules = driver.find_elements(By.CLASS_NAME, 'CourseModulesList_moduleListItem__b8AY9')
